...
'''
def print_tunnel_interface(TunnelID,PeerID):
    print (f'set network interface tunnel units tunnel.{TunnelID} comment PEER-{PeerID}')
    print (f'set network virtual-router default interface tunnel.{TunnelID}')
    print (f'set zone VPN-S2S network layer3 tunnel.{TunnelID}')
    print (f'set import network interface tunnel.{TunnelID}')
...
def print_static_route(PeerID,seq_number,TunnelID,REMOTE_NET_OBJ):
    if '/' not in REMOTE_NET_OBJ:
        REMOTE_NET_OBJ = REMOTE_NET_OBJ + '/32'
...
print('========================')
print('TUNNEL INTERFACE Section')
print('========================')
...
print_tunnel_interface(TunnelID,PeerID)
...
#hosts objects setup
for i in local_host:
    i = 'H-'+i.removesuffix('/32') #<<-- only available in python version > 3.9
...
sorted_local_nets = sorted(local_nets_set)
sorted_remote_nets = sorted(remote_nets_set)
...
seq_number = 1 
for a in sorted_local_nets:
    for b in sorted_remote_nets:
        print_proxy_id(PeerID,seq_number,local_nets_set,remote_nets_set)
        seq_number += 1
print(f'set network tunnel ipsec PEER-{PeerID} auto-key ipsec-crypto-profile <!!!! ENTER IPSEC PROFILE NAME HERE !!!!>')
...
local_nets =  "[ " + " ".join(local_subnet_obj) + " " + " ".join(local_host_obj) + " ]" 
remote_nets = "[ " + " ".join(remote_subnet_obj) + " " + " ".join(remote_host_obj) + " ]"
